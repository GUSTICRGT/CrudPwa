...
import {createMuiTheme, /*emphasize,*/ ThemeProvider, withStyles} from '@material-ui/core/styles';
...
class CambioVehiculo extends Component 
{
    ...
    constructor(props)
    {
        ...
        this.state = {
            ...
            vehiculoStepData: false,
            newVehiculo: null,
            fechaTransaccion: new Date(),
            precio: 50,
            openModal: false,
            colorChipActive: true,
            loading: false,
        };
    }
    ...
    activeUpDownTransaccion = (val) => {
        this.setState({colorChipActive: (val==='up') ? true : false});
    }
    ...
    handleRegistrarCambio = () => {
        ...
        let _newVehiculoSelected = this.state.newVehiculo;
        let _tipoCambioVehiculo = this.state.colorChipActive;
        let cambioNombreTransaccion = new TransaccionesManage(_socioSelect);
        if (_vehiculoSelected.getPlaca() === _newVehiculoSelected.placa){
            // console.log("Error misma placa")
            this.notificationDOMRef.current.addNotification({
                title: "Validación: << Transacción >>",
                message: "El número de placa esta repetido.",
                type: "danger",
                insert: "bottom",
            ...
            cambioNombreTransaccion.cambioVehiculo(_vehiculoSelected, _newVehiculoSelected, _tipoCambioVehiculo)
                .then(() => {
                    // console.log(resp);
                    this.notificationDOMRef.current.addNotification({
                        title: "<< TRANSACCIÓN >>",
                        message: "Cambio de vehículo realizado correctamente",
                        type: "success",
                        insert: "bottom",
                        container: "bottom-right",
                        animationIn: ["animated", "fadeIn"],
                ...
                <Card style={styles.paper}>
                    ...
                    <CardContent>
                        <Breadcrumbs separator={"|"}>
                            <StyledBreadcrumb
                                label={"Subida de modelo"}
                                icon={<VerticalAlignTop/>}
                                onClick={() => this.activeUpDownTransaccion('up')}
                                color={(this.state.colorChipActive) ? 'primary' : 'default'}
                            />
                            <StyledBreadcrumb
                                label={"Rebaje de modelo"}
                                icon={<VerticalAlignBottom/>}
                                onClick={() => this.activeUpDownTransaccion('down')}
                                color={(!this.state.colorChipActive) ? 'primary' : 'default'}
                            />
                        </Breadcrumbs>
                        <br/>
                                            ...
                                            (this.state.vehiculoSelected === null || this.state.vehiculo === '') ?
                                                ...
                                                (
                                                    <div>
                                                        ...
                                                        <br/>
                                                        {
                                                            (this.state.newVehiculo === null) ?
                                                                <HerramientaSteper handleBind={this.handleBindVehiculo}/>
                                                                :
                                                                <div>
                                                                    <ExpansionPanel expanded={true} key={this.state.newVehiculo.placa} style={styles.expandedStyleSuccess}>
                                                                        <ExpansionPanelSummary
                                                                            expandIcon={<ArrowBackIos />}
                                                                            id={this.state.newVehiculo.placa}
                                                                        >
                                                                            <Typography >Vehiculo</Typography>
                                                                        </ExpansionPanelSummary>
                                                                        <ExpansionPanelDetails>
                                                                            <List style={styles.ulLi100}>
                                                                                <ListItem>
                                                                                    <ListItemText primary={ValueUpper(this.state.newVehiculo.placa)} secondary="Nro Placa" />
                                                                                </ListItem>
                                                                                <Divider />
                                                                                <ListItem>
                                                                                    <ListItemText primary={ValueUpper(this.state.newVehiculo.modelo)} secondary="Modelo" />
                                                                                </ListItem>
                                                                                <Divider />
                                                                                <ListItem>
                                                                                    <ListItemText primary={ValueUpper(this.state.newVehiculo.marca)} secondary="Marca" />
                                                                                </ListItem>
                                                                                <Divider />
                                                                            </List>
                                                                        </ExpansionPanelDetails>
                                                                    ...
                                                                    <Button
                                                                        variant="outlined"
                                                                        onClick={ () => this.setState({ newVehiculo: null}) }
                                                                    >
                                                                        Cambiar vehículo
                                                                    </Button>
                                                                    <br/>
                                                                    ...
                                                                    <Dialog fullScreen open={this.state.openModal} TransitionComponent={this.Transition}>
                                                                        ...
                                                                        <Typography align={'center'} color={'primary'} variant={'h3'} >{ (this.state.colorChipActive) ? 'Subida de modelo' : 'Rebaje de modelo' }</Typography>
                                                                        <List>
                                                                            <ListItem>
